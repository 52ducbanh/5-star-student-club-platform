...
- **Core Framework**: React 19.2.8 + React DOM 19.2.8 (`<StrictMode>` enabled).
...
```mermaid
flowchart TD
    IndexHTML["index.html (Meta, Google Fonts, SV5T Favicon)"] --> Main["main.tsx (StrictMode + createRoot)"]
    ...
    RoutesContainer --> HomePage["pages/HomePage.tsx (#gioi-thieu, #hanh-trinh, #hoat-dong, #faq, #lien-he)"]
    ...
    AppShell --> Footer["Footer.tsx (4-Column Desktop / 2-Column Mobile Compact)"]
```
...
2. **Layout Shell**: `AppShell` runs the global Lenis RAF scroll loop, updates route metadata/title, traps focus to `#main-content`, and orchestrates smooth hash scrolling.
...
│   ├── og-5ss-v2.png               # Social share preview (1200x630px)
...
│   │   │   ├── Footer.tsx          # Responsive footer with back-to-top, address & links
...
│   │   │   ├── JourneyPanel.tsx    # Detail accordion panel with roadmap & checklist
...
│   ├── pages/
│   │   ├── ActivitiesPage.tsx      # Activity list + query-param URL modal sync (?item=id)
...
1. **Initialization (`main.tsx` $\rightarrow$ `App.tsx`)**:
   - `LoadingProvider` mounts and sets initial state `isLoaded = false`.
   - `LoadingScreen` renders as a fixed overlay (`z-index: 9999`) and begins its single session loop.
2. **Asset & Readiness Preloading**:
   ...
   - Progress increments monotonically ($0 \rightarrow 100\%$) over a guaranteed minimum time (1600ms first visit, 600ms cached session, 500ms reduced-motion).
3. **Cinematic Handoff & Hero Reveal**:
   - Upon hitting $100\%$, an anticipation charge of 260ms occurs.
...
4. **Runtime User Interactions**:
   - **Lenis Smooth Scroll**: All mouse wheel and anchor clicks scroll smoothly with offset compensation.
   - **Section Spy**: `IntersectionObserver` on `#gioi-thieu`, `#hanh-trinh`, `#faq`, `#lien-he` updates navigation highlight visually without pushing history.
...
| `/hoat-dong` | `ActivitiesPage` | Full news & event archive with category tabs (`Tất cả`, `Sự kiện`, `Tin tức`, `Workshop`), search, and URL modal sync (`?item=id`). |
...
- **`Footer`**: Footer with 4 desktop columns (Brand summary, Khám phá links, Hành trình routes, Contact channels) and compact 2-column mobile stacking.
...
- **`AffiliationMarquee`**: Infinite CSS-animated ticker showcasing student association emblems.
...
### Feature Subsystems
- **`JourneyMap`**: Root state orchestrator for the criteria roadmap. Renders `JourneyConstellation` on desktop ($\ge 769$px) or `JourneyMobileTrack` on mobile ($\le 768$px) alongside `JourneyPanel`.
...
6. `@import "./styles/journey.css";` (Constellation canvas & roadmap styles)
...
1. **`HeroGalaxyScene.tsx`**:
   - **Camera**: Perspective camera at `position: [0, 0, 6.2]`, `fov: 46`.
   ...
   - **Optimization**: Visibility tracked via `IntersectionObserver`. When offscreen or in reduced-motion mode, `frameloop` flips to `"demand"`, halting GPU RAF execution. Mobile DPR is clamped to `[1, 1.25]`.
2. **`Criteria3DScene.tsx`**:
   - **Camera**: Perspective camera at `position: [0, 0, 5.0]`, `fov: 48`.
...
- **Forms**: `ContactForm` and `RegistrationForm` run simulated submissions with realistic timeout feedback.
- **External Links**:
  - Social Channels: Facebook (`facebook.com/5ssuet`), TikTok, Contact Email (`5ss.uet@gmail.com`).
  - Google Maps: Embedded location for 144 Xuân Thủy, Cầu Giấy, Hà Nội.
...
- **`tsconfig.app.json`**: Strict TypeScript configuration with `noEmit: true`, `jsx: react-jsx`, and ES2022 target.
...
- [x] **Loading System**: Monotonic 0–100% asset loader with cinematic hero reveal.
...
- [x] **Journey Map**: 2D constellation map with normalized coordinate grid (1000x640), mobile vertical track, criterion accordion panel, LocalStorage checklist persistence.
- [x] **Activities Page**: Category filter tabs, search filter, URL-synced modal sheets (`?item=id`).
- [x] **FAQ & Contact**: Accordion FAQ + honest simulated contact form + university address.
- [x] **Footer**: 4-column desktop / 2-column mobile compact layout.
...
- Key Feature 3: Deep-linked modal dialogs synced with URL search params (?item=id).
